...
def get_app_root() -> Path:
    ...
    if getattr(sys, 'frozen', False):
        if sys.platform == "darwin":
            # macOS .app：数据放 ~/Library/Application Support/WorkTrace
            # （签名 app 不可写 bundle 内，写 bundle 会破坏代码签名）
            app_support = Path.home() / "Library" / "Application Support" / "WorkTrace"
...
from src.collector.platform_factory import (
    ClipboardMonitor,
    ClipboardRecord,
    IdleDetector,
    IdleState,
...
from src.collector.snapshot_importer import SnapshotImporter
...
class BlackboxEngine:
    ...
    def start(self):
        ...
        # 启动快照采集（混合增强：Git/Shell/浏览器等）
        if self._settings.collection.get("snapshot_enabled", True):
    ...
    def _start_snapshot_loop(self):
        """后台定时跑快照采集（Git/Shell/浏览器/文件），导入主库供日报聚合"""
        import threading
        interval = float(self._settings.collection.get("snapshot_interval_seconds", 1800))
        roots_cfg = self._settings.collection.get("snapshot_roots", [])
        roots = [Path(r) for r in roots_cfg] or [Path.cwd()]
        since = int(self._settings.collection.get("snapshot_since_hours", 24))
        importer = SnapshotImporter(self._db, str(self._settings.db_path), roots, since)
        logger.info("快照采集已启用，roots=%s，间隔 %.0f 秒", roots, interval)
        ...
        def _worker():
            ...
            while self._running:
                try:
                    ...
                    logger.exception("快照后台线程异常")
                wait = 5.0 if first else interval
                first = False
                self._stop_event.wait(wait)

        threading.Thread(target=_worker, daemon=True, name="SnapshotImporter").start()
...
def _macos_permission_hint():
    """macOS 首次运行权限引导（仅提示，不阻断；其他平台空操作）"""
    if sys.platform != "darwin":
        return
    print(
        "\n[macOS 权限提示] 为保证采集完整，请到「系统设置 → 隐私与安全性」授权：\n"
        "  · 输入监控 — 键盘输入采集（CGEventTap）需要；勾选本程序后需完全退出重启\n"
        "  · 辅助功能 — 全局快捷键/窗口切换标题可能需要（可选）\n"
        "  · 屏幕录制 — 前台窗口标题采集需要（仅取标题，不截屏）\n"
        "  · 输入监控 — 若启用键盘增强采集\n"
        "未授权时对应采集会静默降级（如窗口标题为空），不影响其余功能。\n"
    )
