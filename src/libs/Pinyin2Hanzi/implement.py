# coding: utf-8
# Vendored from https://github.com/letiantian/Pinyin2Hanzi (MIT License)
# 修改点：数据文件由 .json 改为 .json.gz（体积 37MB -> 5.8MB）
...
class DefaultHmmParams(AbstractHmmParams):
    ...
    def __init__(self,):
        ...
        self.py2hz_dict      = self.readjson(os.path.join(current_dir, 'data', 'hmm_py2hz.json.gz'))
        self.start_dict      = self.readjson(os.path.join(current_dir, 'data', 'hmm_start.json.gz'))
        self.emission_dict   = self.readjson(os.path.join(current_dir, 'data', 'hmm_emission.json.gz'))
        self.transition_dict = self.readjson(os.path.join(current_dir, 'data', 'hmm_transition.json.gz'))

    def readjson(self, filename):
        with gzip.open(filename, 'rt', encoding='utf-8') as outfile:
...
class DefaultDagParams(AbstractDagParams):
    ...
    def __init__(self,):
        current_dir = self.pwd()
        self.char_dict      = self.readjson(os.path.join(current_dir, 'data', 'dag_char.json.gz'))
        self.phrase_dict    = self.readjson(os.path.join(current_dir, 'data', 'dag_phrase.json.gz'))
