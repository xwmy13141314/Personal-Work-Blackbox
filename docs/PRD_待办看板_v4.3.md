...
| 指标 | 口径 | 色 |
|---|---|---|
| 总任务 | 全部已入库待办（`is_draft=0`） | 中性 |
| 今日待办 | 未完成 且 **未逾期**（`due_date ≥ 今天` 或无截止） | 蓝 |
| 已延期 | 未完成 且 `due_date < 今天` | 🔴 红 |
| 已完成 | `status=done` | 绿 |

> **口径**（已与用户对齐）：「今日待办」**≠今日到期**，而是「还在正常轨道上的待办池」（今天到期 + 未来到期 + 无截止，只要没逾期都算）；「已延期」是其中**以前没做完、已过截止**的子集，单独红色警示。两者互斥，合起来 = 全部未完成。

**P3 · 融合工作实况**（在 P1 卡片旁叠加）— 当日专注时长 + 主要应用类别 + 时间分布环形图（复用 `query_category_stats`）。一眼看全「该做什么 + 实际做了什么」。
...
5. 优先级排序（颜色 + 高→低 + 手动序）
6. 任务统计总览卡片（§4.7 P1）：顶部 4 指标（总任务 / 今日待办 / 已延期 / 已完成）
7. 保留现有：草稿区、手动增删、CSV 导出、AI 提取
...
- [ ] 看板顶部统计卡片显示 4 指标，口径正确（今日待办=未逾期未完成；已延期=逾期未完成，红色高亮）
- [ ] 现有待办（已入库）在看板正确分列显示
...
- `database.get_todo_stats(date)`（4 个 count：总数 / 未逾期未完成 / 逾期未完成 / done）+ web_api 桥接
...
### P3 · 概览融合 + 跨日迁移
- 任务概览叠加工作实况（§4.7 P3：专注时长 / 主要应用 / 时间分布环形图，复用 `query_category_stats`）
- 跨日自动迁移（§4.8）
- 桌面 toast 提醒（§4.9）
