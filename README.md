...
- **AI 周度洞察**：报告页「洞察」tab，对比本周 vs 上周活动数据（小时分布 / 每日时长 / 分类占比）生成最佳时段 / 风险信号 / 周环比 / 目标节奏四类洞察卡片；LLM 不可用时自动降级本地统计规则
- **Web GUI**：macOS 风格三栏界面（pywebview + React），Windows 原生窗口
- **八视图导航**：驾驶舱 / 速记 / 报告 / 统计 / 活动 / 待办 / 设置 / 关于（工作台 / 记录与报告 / 系统 三组）
- **常驻日历**：标记有采集（蓝点）与有日报（底色）的日期，周 / 月切换
- **全局搜索（Ctrl+K）**：任意界面唤起命令面板，跨报告 / 待办 / 速记 / 输入记录四类检索，点击结果直达对应模块；活动页支持按日期 / 应用定位历史输入
...
- **IME 中文捕获**：智能识别中文输入法组合文本，记录汉字而非拼音；数字键选字（候选 0-9）同样触发上屏捕获
- **真实上屏文本采集**（v5.3.0）：提交入库前用 **UIA 快照**（独立子进程，Value/Text/MSAA 三路读取焦点控件、基线差分新增内容）与 **WPS/Office COM**（ActiveCell / Selection 单元格级读取，覆盖 WPS 表格这类 UIA 读不到的控件）替换键盘文本——记录的是**真实上屏内容**而非键盘逆推；隐私模式/黑名单自动跳过，`accurate_mode` 可一键回退纯键盘链路
- **拼音智能识别 2.0**：IME 未捕获的拼音记录，活动页一键转换汉字——离线分层引擎（Pinyin2Hanzi：DAG 词库 → HMM 维特比 → 单字映射兜底，纯本地，**部分转换**：能识别的音节块转汉字、简拼等识别不了的保留原文）+ 可选「AI 增强」（LLM 批量重识别，支持简拼/混拼还原，效果最准，结果带 Sparkles 高亮标识）；转换仅在展示层，原始数据不变；残缺拼音低置信度自动保留原文不硬转
- **速记标签与洞察收件箱**（v5.4.0）：速记支持标签（标签云 + 点选筛选 + 搜索命中标签）；可配置「洞察收件箱」目录，速记保存时**双写**——既入本地数据库，又落盘为带 frontmatter 的 Markdown，直接喂给「每日洞察」类 AI 蒸馏工作流；全局快捷键 **Ctrl+Alt+I** 任意界面呼出速记
- **隐私保护**：首次启动告知弹窗 + 三层过滤 + 一键隐私模式 + 可选数据库加密（SQLCipher）
- **国产模型友好**：设置页下拉预设，填 Key 即用

## 界面预览

**活动明细** — 按日期浏览会话，应用自动分类 + 使用时长 + 输入文本片段；左栏搜索框可跨日期全文检索历史输入

![活动明细](docs/screenshots/activity.jpg)

**待办跟进** — AI 从报告自动提取待办（状态 / 优先级 / 来源），支持一键导出 CSV（UTF-8 BOM，Excel 中文不乱码）

![待办跟进](docs/screenshots/todo.jpg)

**时间分布环形图** — LLM 从报告文字提取各类工作占比，纯 SVG 渲染（无 JS 依赖），app 内与导出 HTML 共用同一处生成

![时间分布环形图](docs/screenshots/timedist-donut.svg)

**关于** — 版本信息 + 隐私承诺（本地存储 / 三层过滤 / 开源可审计 / 可选数据库加密）+ 联系方式

![关于](docs/screenshots/about.jpg)
...
| 栏 | 内容 |
|----|------|
| 左栏 | 分组导航（工作台：驾驶舱 / 速记 · 记录与报告：报告 / 统计 / 活动 / 待办 · 系统：设置 / 关于）+ 搜索框 + Logo |
| 中栏 | 当前视图内容 |
| 右栏 | 录制控制（启动 / 暂停 / 停止 / 隐私）+ 状态 + 日历 |

### 报告视图
日 / 周 / 月报 + 洞察四 tab 切换，日期导航，点「生成报告」由 LLM 生成（Markdown 渲染）；「洞察」tab 一键生成 AI 周度洞察（LLM 优先，本地统计兜底，结果缓存至 `data/insights/`）。录制中生成日报会自动包含当前会话内容。任意界面 `Ctrl+K` 唤起全局搜索命令面板。
...
│   └── pinyin_converter.py # 拼音转汉字分层引擎（DAG→HMM→单字映射）
├── libs/Pinyin2Hanzi/   # 内置拼音转换引擎（vendored, MIT, 数据 gzip 5.8MB）
...
### v5.4.0 (2026-09-23) — 速记标签化 + 洞察收件箱双写

- **速记标签**：`notes` 增加 `tags`；标签输入（自动去重归一）、卡片标签 chip、**标签云**（按次数排序）、**点标签精确筛选**（`竞品` 不误中 `竞品分析`）；搜索同时命中内容与标签
- **洞察收件箱双写**：速记保存时同步落盘 Markdown（`type/created/tags/source` frontmatter）到可配置目录，供「每日洞察」AI 蒸馏流程增量消费；留空 = 仅入库；面板显示连通性 / 可写性 / 待处理文件数，保存后**热生效**无需重启
- **速记页四指标**：今日 / 本周（周一为始）/ 累计 / 置顶
- **全局快捷键 Ctrl+Alt+I**：任意界面呼出速记；Web UI 模式下 `Ctrl+Alt+P/R/N/I` 全部可用
- **修复上游仓库缺陷**：拼音 HMM/DAG 词典被 `.gitignore` 的 `data/` 规则误排除导致新克隆仓库拼音识别退化（8 个用例失败），已补回 6 个词典文件（5.9MB，vendored · MIT）并加忽略例外
- **旧库无损升级**：启动时自动把旧 `insights` 表（v4.5 并行开发线的速记）搬移到 `notes`（幂等、只搬一次、原表保留），老用户升级后历史速记不丢
- 测试基线 539 passed（本机全量、0 failed）

### v4.4.0 (2026-08-14) — 待办删除归档

- **删除不再丢数据**：待办改为软删除（`deleted_at` 时间戳），主界面不显示但记录保留，随时可找回
- **文件归档双保险**：删除时同步追加 `data/exports/todo_archive_YYYY-MM.md`（按月人读表格）+ `todo_archive.jsonl`（全字段机读），append-only 永不回改
- **归档视图**（待办 → 视图 → 归档）：浏览已删待办、关键词搜索（标题/备注）、分页加载
- **一键恢复**：回看板原位置（sort_order 保留）；**彻底删除**：两步确认后真删（归档文件历史仍保留）
- 看板/统计/导出/AI 提取去重口径全部排除已删待办；测试基线 364 passed

### v4.3.2 (2026-08-14) — UI 换肤

- **视觉方案落地**（依据 `职迹UI优化效果图` 设计稿）：品牌色 `#0071e3`→`#2563eb`、背景 `#f3f4f6`、Inter 字体栈，`theme.css` token 全量更新（各视图自动跟随）
- **日报页**：工具条实色白底（日/周/月分段器、HTML/PDF 描边按钮、生成报告按钮）；右侧栏 320px 实色白卡
- **设置页**：实色白卡 + provider 方形描边 chip（标签「智谱 GLM」等）、输入框 focus 双环、内容限宽
- **字体大小默认「中」**（原默认「小」），小/中/大三档切换不变（即时生效 + 记忆）
- **导航**：删除「待办」的「新」徽标；左导航保留原浅色毛玻璃配色（宽 240px）
- 版本号统一为 4.3.2；测试基线 353 passed
